  ...
  return (
    <div className="set-section">
      ...
      <div className="set-card">
        <label className="set-card-row">
          <span className="set-label-stack">
            <span className="set-label">Keep history for</span>
            <span className="set-sublabel">Pinned items never expire</span>
          </span>
          <select
            className="set-input set-input-compact"
            value={settings.clipboard.retention_days}
            onChange={(event) => updateClipboard({ retention_days: Number(event.target.value) })}
          >
        ...
        <label className="set-card-row">
          <span className="set-label-stack">
            <span className="set-label">Unpinned item limit</span>
            <span className="set-sublabel">Oldest items leave first</span>
          </span>
          <select
            className="set-input set-input-compact"
            value={settings.clipboard.max_unpinned_items}
            onChange={(event) =>
              updateClipboard({ max_unpinned_items: Number(event.target.value) })
            }
          >
        ...
        <label className="set-card-row">
          <span className="set-label-stack">
            <span className="set-label">Storage limit</span>
            <span className="set-sublabel">Images count toward this limit</span>
          </span>
          <select
            className="set-input set-input-compact"
            value={settings.clipboard.max_storage_mb}
            onChange={(event) => updateClipboard({ max_storage_mb: Number(event.target.value) })}
          >
      ...
      <p className="set-hint">
        History is saved on this machine so it survives a restart. Anything you copy, including
        passwords and one-time codes, is written to a file in Synapse's data folder. Turn the switch
        off to stop recording immediately; existing entries stay until you clear them.
      </p>
    </div>
  );
}
