...
plus the `update-feed` guard.

**The release signing key is the maintainer's, and only the maintainer's.**
`plugins.updater.pubkey` started as the placeholder
`REPLACE_WITH_TAURI_SIGNING_PUBLIC_KEY`, which the `update-feed` guard fails on
by design — a build whose only integrity check cannot pass must not ship. It now
holds a minisign public key generated on the maintainer's machine; the private
half lives at `~/.tauri/synapse.key` there and as the `TAURI_SIGNING_PRIVATE_KEY`
repository secret, which is what `release.yml` signs with.

**v0.2.0 is the first signed release, and the signing chain is verified.** The
published installer was checked independently of Tauri: the key id in
`plugins.updater.pubkey` matches the key id in the shipped signature
(`d66a7cd7436241c7`), and the Ed25519 signature verifies against BLAKE2b-512 of
the installer bytes. So the key CI signs with and the key the app verifies
against are provably the same key — the failure mode where those drift produces
a release that publishes fine and then refuses to install for everyone.
`latest.json` resolves through `releases/latest/download/`, non-draft, so the
endpoint the app queries will not 404.

What that does **not** cover is runtime behaviour: whether the plugin accepts it
in-process, whether the app relaunches after install (`app.restart()` may never
execute on Windows — the installer closes the app itself), and whether
`.update-pending` keeps onboarding suppressed. Those need a real v0.2.0 ->
v0.2.1 update, which is what this release is for.

This is a key-custody rule, not a preference. Whoever holds the private half of
the pinned key can sign an installer that every Synapse install will accept and
silently execute — so it cannot be generated by, transmitted to, or shared with
anyone outside the project, however convenient that would be. A contributor
generated a second keypair while this branch was in flight and pinned its public
half here; it was superseded rather than adopted, for that reason, and that key
signs nothing. If the pinned key ever needs replacing, generate a new one with
`npm run tauri signer generate`, update this field and the repository secret
together, and cut a release — installs on the old key cannot auto-update across
the change and will need a manual installer download.
